...
- **CORS**
    ...
    - **📚 블로그 읽고 교차 출처 리소스 공유(CORS) 정리해보기 🍠**
        ...
        - 브라우저에서 `http://localhost:8080` 애플리케이션이 `http://localhost:8081/resource.json`을 요청했을 때, 네트워크 요청과 응답은 어떻게 처리되며, 브라우저가 응답 본문을 사용하지 못하는 이유는 무엇인가요?
            ...
            http://localhost:8080의 브라우저가 http://localhost:8081로 요청을 보내면, 네트워크 요청 자체는 서버에 정상적으로 도달한다. 서버도 요청을 처리하고 200 OK 응답과 JSON 데이터를 브라우저로 다시 보낸다. 하지만 브라우저는 **동일 출처 정책(SOP)**에 따라 요청을 보낸 출처(8080)와 응답이 온 출처(8081)가 다름을 확인하고, 8081 서버의 응답 헤더에 "이 요청을 허용한다"는 CORS 헤더가 없으면 보안을 위해 응답 본문을 차단한다.
            
        - 서버가 다른 출처(`http://localhost:8080`)에서 자원을 사용할 수 있게 하려면 어떤 응답 헤더를 어떻게 설정해야 하나요? 글의 예시 코드를 참고해 헤더 이름과 값까지 구체적으로 쓰세요.
            
            서버가 다른 출처([http://localhost:8080](http://localhost:8080/))의 요청을 허용하려면, 응답 헤더에 Access-Control-Allow-Origin을 포함해야 한다. 만약 8080만 특정해서 허용하려면 **Access-Control-Allow-Origin: [http://localhost:8080](http://localhost:8080/)**이라고 설정한다. 만약 모든 출처를 허용하려면 Access-Control-Allow-Origin: *를 사용하지만, 보안상 권장하지 않는다.
            
        - 단순 요청으로 분류되기 위해서는 어떤 두 가지 조건을 만족해야 하나요? 또한 `GET /resource.json` 요청이 단순 요청에 해당하는 이유를 설명하세요.
            
            단순 요청(Simple Request)으로 분류되려면 두 가지 조건을 만족해야 한다. 첫째, 메서드가 GET, HEAD, POST 중 하나여야 한다. 둘째, Accept, Content-Type 등 정해진 기본 헤더 외에 커스텀 헤더를 사용하지 않아야 한다. GET /resource.json 요청은 메서드가 GET이고 별다른 커스텀 헤더를 추가하지 않았기 때문에 단순 요청에 해당한다.
            
        - 브라우저에서 `X-Goguma`라는 커스텀 헤더를 추가했을 때 왜 차단이 발생하나요? 이 문제를 해결하기 위해 서버에서 추가해야 하는 응답 헤더와 값은 무엇인가요?
            
            X-Goguma는 기본 헤더가 아닌 커스텀 헤더이므로, 이 요청은 '단순 요청'의 조건을 벗어난다. 브라우저는 이 요청이 안전한지 확인하기 위해 먼저 **사전 요청(Preflight)**인 OPTIONS 요청을 서버로 보낸다. 이 문제를 해결하려면 서버는 OPTIONS 요청의 응답 헤더에 Access-Control-Allow-Headers: X-Goguma 헤더를 추가하여 "그 헤더는 사용해도 좋다"고 허용해줘야 한다.
            
        - `PUT` 요청을 보낼 때 브라우저는 왜 먼저 `OPTIONS` 요청을 보내나요? 이때 브라우저가 보내는 헤더와 서버가 응답해야 하는 헤더를 각각 쓰고, 사전 요청과 실제 요청이 어떻게 이어지는지 간단히 서술하세요.
            
            이유: PUT 메서드는 '단순 요청'의 메서드(GET, POST, HEAD)에 포함되지 않는다. 따라서 브라우저는 안전하지 않을 수 있다고 판단하여, 실제 PUT 요청을 보내기 전에 먼저 **사전 요청(OPTIONS)**을 보내 서버가 PUT을 허용하는지 검사한다. 
            
            헤더: 브라우저는 OPTIONS 요청에 Access-Control-Request-Method: PUT 헤더를 담아 보낸다. 서버는 이에 대한 응답으로 Access-Control-Allow-Methods: PUT (또는 GET, POST, PUT) 헤더를 보내야 한다. 
            
            흐름: 브라우저가 OPTIONS 요청을 먼저 보내고 서버로부터 "허용한다"는 응답(200 OK + Allow 헤더)을 받으면, 그때서야 비로소 원래 보내려던 실제 PUT 요청을 이어서 보낸다.
    ...
    **RBAC과 ABAC은 모두 '누가', '무엇을' 할 수 있는지 결정하는 접근 제어(Access Control) 모델이에요.**
    ...
    - **📚 블로그 읽고 ABAC 정리해보기 🍠**
        ...
        - **RBAC**의 한계에 대해 설명해주세요.
            ...
            RBAC은 역할(Role)에만 권한이 묶여있어 유연성이 떨어진다. '팀장' 역할이라도 '자기 팀 문서만 읽기' 또는 '업무 시간 외 접근 금지'처럼 세밀하고 동적인 조건을 설정하기 어렵다. 이런 요구사항이 늘어나면 역할의 수가 지나치게 많아지는 '역할 폭발(Role Explosion)' 문제가 발생한다.
            
        - **ABAC**으로의 전환, 어떤 '기준'이 적절할까요?
            
            기존 RBAC의 역할이 너무 많아져 관리가 복잡해질 때가 첫 번째 전환 기준이다. 또한 "A팀의 팀장은", "업무 시간에만", "내부망에서만" 접근 가능해야 한다는 등, 역할 외에 사용자, 리소스, 환경의 속성을 복합적으로 따져야 하는 권한 정책이 필요할 때 ABAC 도입을 결정한다.
            
        - 어떤 서비스 영역에 **RBAC**을 남겨두고, **ABAC**을 도입하시겠어요?
            
            권한 구조가 단순하고 고정적인 영역은 RBAC을 남겨둔다(예: '슈퍼 관리자'와 '일반 사용자'를 구분하는 어드민 페이지 접근). 반면, '결제 정보', '개인정보', '프로젝트별 문서'처럼 데이터의 민감도가 높거나 사용자의 상황(시간, 위치)에 따라 접근을 세밀하게 제어해야 하는 서비스 영역에는 ABAC을 도입한다.
            
        - 여러분들은 다른 부서에서 요청을 받았을 때 어떤식으로 행동하실껀가요?
            
            요청을 받으면 "왜 이 권한이 필요한가?"라는 비즈니스 목적과 배경을 먼저 명확히 파악한다. 그 후, '최소 권한의 원칙(PoLP)'에 따라 요청받은 권한 전체가 아닌, 해당 업무를 수행하는 데 꼭 필요한 최소한의 권한(예: '쓰기' 대신 '읽기')만 부여하는 것을 원칙으로 한다.
...
- [x]  Swagger를 확인 후, **Token이 필요한 (자물쇠가 걸린) API**에만 방문할 수 있는 페이지인 경우 `Protected Route`를 활용해 경로를 보호했나요?

**리다이렉트 처리**

- [x]  사용자가 로그인하지 않은 경우, 인증이 필요한 페이지에 접근 시 **로그인 페이지로 Redirect** 되게 설정했나요?
...
- [x]  아래 문서를 보고, 서버 세팅을 완료했나요?
    
    [RefreshToken 실습 전 확인해주세요!](https://www.notion.so/RefreshToken-283b57f4596b81eda2aed71b3e155acd?pvs=21)
    

**토큰 갱신 로직**

- [x]  **Access Token** 만료 시, **Axios의 응답 인터셉터**를 활용하여 Refresh Token으로 새로운 Access Token을 발급받고 실패한 요청을 **재시도**하는 로직을 작성했나요?

**무한 루프 방지**

- [x]  토큰 갱신 과정에서 `_retry` 플래그 등을 사용하여 **무한 재시도가 발생하지 않도록** 방지했나요?

**동작 검증**

- [x]  서버 토큰 시간을 임의로 짧게 조정을 해도 토큰 재발급 요청이 **정상적으로 동작**하여 서비스를 끊김 없이 이용할 수 있나요?
...
- [x]  아래 문서를 보고, Google API Console 설정 및 서버 세팅을 완료했나요?
    
    [Google Login 실습 전 확인해주세요!](https://www.notion.so/Google-Login-283b57f4596b81feab87ef5c7d1d7969?pvs=21)
    
- [x]  Client ID를 발급받고, **Redirect URI**와 **요청할 권한**을 정확히 서버 코드 환경변수에 입력했나요?

**동작 확인**

- [x]  소셜 로그인이 정상적으로 잘 동작하나요?
...
[**인증 인가 시스템, 직접 구축 vs. SaaS 솔루션: 전략적 선택**](https://www.notion.so/vs-SaaS-283b57f4596b8171aaa8e1912609c223?pvs=21)
